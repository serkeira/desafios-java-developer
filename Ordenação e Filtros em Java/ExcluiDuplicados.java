...
public class ExcluiDuplicados {
    ...
    public static void main(String[] args) {
        ...
        Integer quantidadeLinhas = sc.nextInt();
        Integer count = 0;
        String linhaAtual = "";
        ...
        while (count < quantidadeLinhas) {
            ...
            if (linhaAtual.length() > 1) {
                linhaAtual = Arrays.stream(linhaAtual.split(" ")).distinct().sorted().collect(Collectors.joining(" "));
                listasDeCompras.add(linhaAtual);
                count++;
            }
        }

        for (String lista : listasDeCompras) {
